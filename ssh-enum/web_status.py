...
포트 8082 에서 SSH 서비스 상태 및 취약점 정보를 제공합니다.
"""
import http.server
import subprocess
import socket
...
<div class="container">
  ...
  <div class="card">
    ...
    <div class="status-row">
      <div class="dot {ssh_dot}"></div>
      <span>SSH 서비스 (포트 22): <strong>{ssh_status}</strong></span>
    </div>
    <div style="margin-top:8px;font-size:0.85rem;color:#8b949e">
      OpenSSH 버전: <strong style="color:#e6edf3">{ssh_version}</strong>
    </div>
  </div>
...
이를 통해 공격자는 유효한 사용자 이름을 사전 대입으로 열거 가능.</pre>
  ...
  <div class="card">
    ...
    <div class="cmd"># python3 ssh_user_enum.py --target localhost --port 2222 --userlist users.txt</div>
...
def get_test_users():
    ...
    for u in target:
        try:
            pwd.getpwnam(u)
            badges += f'<span class="user-badge">{u}</span>\n'
        except:
            pass
    return badges or "<span style='color:#8b949e'>계정 정보를 불러올 수 없습니다.</span>"
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        html = HTML.format(
            ssh_dot='green' if ssh_up else 'red',
            ssh_status='실행 중 ✓' if ssh_up else '중지됨 ✗',
            ssh_version=get_ssh_version(),
            user_badges=get_test_users(),
        )
...
if __name__ == '__main__':
    ...
    print("SSH Enum web UI: http://0.0.0.0:8082")
